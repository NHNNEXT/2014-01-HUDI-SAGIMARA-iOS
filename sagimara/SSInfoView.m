...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [photoView setFrame:CGRectMake(20, 80, 100, 100)];
        
        nameLabel  = [self makeLabel:nameLabel x:20 y:180 width:100 height:30 text:@"홍 길동" size:14.f];
        [nameLabel setTextAlignment:NSTextAlignmentCenter];
        
        gradeLabel = [self makeLabel:gradeLabel x:130 y:80 width:180 height:30 text:@"Trust" size:23.f];
        [gradeLabel setTextAlignment:NSTextAlignmentCenter];

        dateLabel = [self makeLabel:dateLabel x:130 y:110 width:180 height:20 text:@"2014년 3월 4일 인증됨" size:10.f];
        [dateLabel setTextAlignment:NSTextAlignmentCenter];
        
        detailLabel = [self makeLabel:detailLabel x:133 y:143 width:174 height:64 text:@"아무나 믿지마세요!" size:14.f];
        
        
        background = [[UIView alloc]initWithFrame:CGRectMake(130, 140, 180, 70)];
